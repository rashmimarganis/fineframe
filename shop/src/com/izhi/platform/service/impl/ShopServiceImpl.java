...
public class ShopServiceImpl extends BaseService implements IShopService {
	...
	public IShopDao getOrgDao() {
		return orgDao;
	}

	public void setOrgDao(IShopDao dao) {
		this.orgDao = dao;
	}
	...
	public Integer save(Shop obj, String oldName) {
		...
		// add
		if (obj.getShopId() == 0) {
			boolean exist = this.orgDao.findIsExist("name", obj.getShopName());
			if (exist) {
				// name exist
			...
			if (obj.getShopName().equals(oldName)) {
				...
				// success
				return new Integer(1);
			} else if (!this.orgDao.findIsExist("name", obj.getShopName())) {
				this.orgDao.update(obj);
				// success
				return new Integer(1);
			} else {
	...
	public List<Shop> findChildren(Integer id) {
		List<Shop> list = orgDao.findChildren(id);
		return list;
	}
	...
	public Map<String, Object> saveShop(Shop obj, String oldName) {
		boolean exist = false;
		if (obj.getShopId() == 0) {
			exist = this.findIsExist("name", obj.getShopName());
			if (!exist) {
				if (obj.getParent() == null || obj.getParent().getShopId() == 0) {
					obj.setParent(null);
				}
			...
			} else if (!this.findIsExist("name", obj.getShopName())) {
				this.orgDao.update(obj);
			} else {
				exist = true;
			}

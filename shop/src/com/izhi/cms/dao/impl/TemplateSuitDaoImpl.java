package com.izhi.cms.dao.impl;

import java.util.List;

import org.hibernate.Query;
import org.hibernate.Session;
import org.hibernate.criterion.DetachedCriteria;
import org.hibernate.criterion.Order;
import org.springframework.orm.hibernate3.support.HibernateDaoSupport;
import org.springframework.stereotype.Service;

import com.izhi.cms.dao.ITemplateSuitDao;
import com.izhi.cms.model.TemplateSuit;
import com.izhi.platform.util.PageParameter;
@Service("templateSuitDao")
public class TemplateSuitDaoImpl extends HibernateDaoSupport implements ITemplateSuitDao {

	@Override
	public boolean deleteTemplateSuit(int id) {
		String sql="delete from TemplateSuit o where o.suitId=? ";
		int i=this.getHibernateTemplate().bulkUpdate(sql, id);
		return i>0;
	}

	@Override
	public boolean deleteTemplateSuits(List<Integer> ids) {
		String sql="delete from TemplateSuit o where o.suitId in(:ids)";
		Session session=this.getSession();
		Query q=session.createQuery(sql);
		q.setParameterList("ids", ids);
		int i=q.executeUpdate();
		return i>0;
	}

	@Override
	public TemplateSuit findTemplateSuitById(int id) {
		return (TemplateSuit)this.getHibernateTemplate().load(TemplateSuit.class, id);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<TemplateSuit> findPage(PageParameter pp) {
		String sortField=pp.getSort();
		String sort=pp.getDir();
		int maxResult=pp.getLimit();
		int firstResult=pp.getStart();
		DetachedCriteria dc = DetachedCriteria.forClass(TemplateSuit.class);
		if (sort != null && sortField != null) {
			sort = sort.toLowerCase();
			if (sort.equals("desc")) {
				dc.addOrder(Order.desc(sortField));
			} else if (sort.equals("asc")) {
				dc.addOrder(Order.asc(sortField));
			}
		}
		if(maxResult==0){
			maxResult=10;
		}
		return this.getHibernateTemplate().findByCriteria(dc, firstResult,
				maxResult);
	}

	@SuppressWarnings("unchecked")
	@Override
	public int findTotalCount() {
		String sql="select count(*) from TemplateSuit";
		List l=this.getHibernateTemplate().find(sql);
		return ((Long)l.get(0)).intValue();
	}

	@Override
	public int saveTemplateSuit(TemplateSuit obj) {
		int i=(Integer)this.getHibernateTemplate().save(obj);
		return i;
	}

	@Override
	public boolean updateTemplateSuit(TemplateSuit obj) {
		this.getHibernateTemplate().update(obj);
		return true;
	}

	@SuppressWarnings("unchecked")
	@Override
	public TemplateSuit findTemplateSuitByName(String name) {
		String sql="from TemplateSuit o where o.suitName=?";
		List<TemplateSuit> l=this.getHibernateTemplate().find(sql,name);
		if(l.size()>0){
			return l.get(0);
		}
		return null;
	}

	@SuppressWarnings("unchecked")
	@Override
	public TemplateSuit findDefaultTemplateSuit() {
		String sql="from TemplateSuit o where o.default=true";
		List<TemplateSuit> l=this.getHibernateTemplate().find(sql);
		if(l.size()>0){
			return l.get(0);
		}
		return null;
	}

}

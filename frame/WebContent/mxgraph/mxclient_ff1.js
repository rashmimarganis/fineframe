// You are licensed to use this software under the terms of the JGraph Evaluation License.// Do *not* use local copies of this code.// Agent: Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.1.3) Gecko/20090824 Firefox/3.5.3 GTB5
...
//mxGraphView 视图
{
    ...
    mxGraphView.prototype.getPerimeterBounds = function(terminal, edge, isSource){
        ...
        if (edge != null) {
            ...
            border += parseFloat(edge.style[(isSource) ? mxConstants.STYLE_SOURCE_PERIMETER_SPACING : mxConstants.STYLE_TARGET_PERIMETER_SPACING] ||
            0);
        }
        if (terminal != null) {
...
//mxGraph
{
    function mxGraph(container, model, renderHint){
        ...
        if (!isIe) {
            ...
            if (renderHint == mxConstants.RENDERING_HINT_EXACT && isIe) {
    ...
    mxGraph.prototype.getSelectionCellsForChanges = function(changes){
		alert("getSelectionCellsForChanges");
        var cells = new Array();
        ...
        marker.getCell = function(evt, cell){
            ...
            if (cell != null) {
                if (self.isConnecting()) {
                    ...
                    if (!self.isValidSource(cell)) {
                        cell = null;
                    }
            }
            else 
                if (self.isConnecting() && !self.isCreateTarget() && !self.graph.allowDanglingEdges) {
                    self.error = '';
                }
            return cell;
        };
    ...
    mxEdgeHandler.prototype.mouseDown = function(sender, me){
        if (me.getState() == this.state) {
            if (me.getHandle() == mxEvent.LABEL_HANDLE && !this.graph.isLabelMovable(me.getCell())) {
                ...
                if (this.graph.isEnabled() && !this.graph.isForceMarqueeEvent(me.getEvent()) && this.marker != null && me.getHandle() != null) {
                    this.start(me.getX(), me.getY(), me.getHandle());
                    me.consume();
                }
        }

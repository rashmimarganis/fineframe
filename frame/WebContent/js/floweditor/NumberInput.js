...
NumberInput.prototype.createControl = function(name, 
                                               width, 
											   value,
											   onChangeHandler,
											   min,
  ...
  var container = DOM.createElement("DIV", "NumberInput");
...
NumberInput.numberUp = function(event) {
  ...
  if (isNaN(val)) input.value = 1;
  else {
  	if (!isNaN(max)) input.value = val>=max ? max : val+1;
...
Jalava._modules['NumberInput'] = true; 

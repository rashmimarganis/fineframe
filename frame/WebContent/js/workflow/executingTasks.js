		...
		initStore:function(){
			store = new Ext.data.Store({
		        proxy: new Ext.data.HttpProxy({
		            url: '../flow/task/myNewTasks.do?typeId='+typeId
		        }),
		        reader: new Ext.data.JsonReader({
		            ...
		            id: 'taskID',
		            fields: [
		                'taskID', 'startTime','flowDeployName','flowDriverName'
		            ]
		        }),
		        remoteSort: true
		    });
		...
		initTree:function(){
		    westPanel = new Ext.tree.TreePanel({
		        ...
		        loader: new Ext.tree.TreeLoader({
		            dataUrl:'../flow/type/executingTaskTypes.do'
		        })
		    });
		        ...
		        'selectionchange' : function(sm, node){
					if (node.id != '0') {
						store.proxy = new Ext.data.HttpProxy({
							url: '../flow/task/myExecutingTasks.do?typeId=' + node.id
						});
						typeId=node.id;

...
var App= function(){
	...
	return {
		...
		initGridPanel:function(){
		    ...
		    grid = new Ext.grid.GridPanel({
				...
				tbar:[{
		            ...
		            scope: this,
					handler:App.showFunctionTreeDlg
				}],
		...
		,
		/*显示功能窗口*/
		showFunctionTreeDlg:function(){
			var roleId=sm.getSelected().get('id');
			if(!roleTreeLoader){
				roleTreeLoader=new Tree.TreeLoader({
					dataUrl:'../function/findByRole.do?roleId='+roleId,
					uiProviders: {
		                'col': Ext.tree.CheckNodeUI
		            }
		        });
			}else{
				roleTreeLoader.dataUrl='../function/findByRole.do?roleId='+roleId;
			}
			
			if(roleMenuTree){
				roleMenuTree.loader=roleTreeLoader;
				var root=roleMenuTree.getRootNode();
				root.reload(function(){
					roleMenuTree.expandAll();
				});
			}
			
			
			if(!menuTreeDlg){
				App.initRoleMenuTree();
				roleMenuTree.expandAll();
				menuTreeDlg=new Ext.Window({
	                el:'menuTreeDlg',
	                layout:'fit',
					title: '岗位分配功能权限',
	                width:450,
			...
			menuTreeDlg.show(this);
		},
		saveRoleMenu:function(){
			var roleId=sm.getSelected().get('id');
			var ids=getSelected();
			var url='../function/saveRoleFunction.do';
			Ext.Ajax.request({
				url:url,
				params:"roleId="+roleId+"&"+ids,
				success:success,
				failure:failure,
				method:'POST'
			});
			function success(rep){
				//Ext.Msg.alert("保存功能",'保存成功！&nbsp;&nbsp;&nbsp;');
				menuTreeDlg.hide();
				//xgAhdlg("保存成功！");
				Ext.Msg.alert('信息','保存成功');
			}
			function failure(rep){
				Ext.Msg.alert("保存功能",'保存失败！');
				Ext.Msg.alert("保存功能",rep.responseText);
			}
		}
		,
		/*初始化当前岗位对应的菜单树*/
		initRoleMenuTree:function(){
			if(!roleMenuTree){
				roleMenuTree = new Ext.tree.CheckBoxTree({
					id:'roleMenuTree',
					layout:'fit',
			        el:'roleMenuTree',
			        autoScroll:true,
			...
			var root = new Tree.AsyncTreeNode({
		        ...
		        draggable:false,
		        id:'0',
				uiProvider:Ext.tree.CheckNodeUI
		    });
			
			roleMenuTree.loader=roleTreeLoader;
		    roleMenuTree.setRootNode(root);
		    roleMenuTree.render();
		}
	};
}();

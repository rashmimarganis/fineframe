...
public class FunctionDaoImpl extends BaseDaoImpl<Function, Integer> implements IFunctionDao {
	...
	public List<Map<String, Object>> findMenus(int orgId, int userId, int pid) {
		...
		String sql="select distinct new map(f.functionId as id,f.functionName as text,f.url as url) from Function f join f.roles r  join r.users u where f.menu=true and r.org.id=? and u.userId=? and f.parent.functionId=? order by f.sequence desc ";

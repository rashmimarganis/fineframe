...
<div id="menu">
	<h3>
	<a href="${ctx}/user/user.do">帐号列表</a>
	<a href="${ctx}/user/role.do">角色列表</a> 
...
<table>
	<tr>
		<th><a href="user.do?page.orderBy=loginName&page.order=
		<s:if test="page.orderBy=='loginName'">${page.inverseOrder}</s:if><s:else>desc</s:else>">
		<b>登录名</b></a></th>
		<th><a href="user.do?page.orderBy=name&page.order=
		<s:if test="page.orderBy=='name'">${page.inverseOrder}</s:if><s:else>desc</s:else>">
		<b>姓名</b></a></th>
		<th><a href="user.do?page.orderBy=email&page.order=
		...
		<tr>
			...
			<td>&nbsp; 
				<security:authorize ifAnyGranted="A_MODIFY_USER">
					<a href="input.do?id=${id}&page.pageRequest=${page.pageRequest}">修改</a>、
					<a href="delete.do?id=${id}&page.pageRequest=${page.pageRequest}">删除</a>
				</security:authorize>
			</td>
		</tr>
	</s:iterator>
...
<div id="footer">
	...
	<s:if test="page.hasPre">
		<a href="user.do?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}">上一页</a>
	</s:if>
	<s:if test="page.hasNext">
		<a href="user.do?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}">下一页</a>
	</s:if>
	<security:authorize ifAnyGranted="A_MODIFY_USER">
		<a href="input.do">增加新用户</a>
	</security:authorize>
</div>
